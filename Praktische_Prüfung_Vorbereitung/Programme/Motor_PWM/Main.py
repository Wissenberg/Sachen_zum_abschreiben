...
class DBMapper:
    ...
    def open_db_connection(self):
        try:
            self.con = sqlite3.connect('testat5.db')
            self.cur = self.con.cursor()
        except ConnectionError:
            self.open_db_connection()   

    def writeKommand(self, robo_id, move, turn):
        self.cur.execute(f"INSERT INTO kommando (robo_id, move, turn, timestamp) VALUES ({robo_id},{move},{turn},'{datetime.datetime.now()}');")
        self.con.commit()
...
class MqttReader:
    ...
    def on_message(self, client, userdata, msg):
        ...
        robo_id = self.db.getRoboId("Robo_Cedric")
