...
class DBMapper:
    ...
    def open_db_connection(self):
        try:
            self.con = sqlite3.connect('kommandos.db')
...
class MqttReader:
    ...
    def on_message(self, client, userdata, msg):
        ...
        if move1 == '1' or move1 == '-1':
            robo_id = self.db.getRoboId(ROBO_NAME, str(msg.topic))
            self.db.writeKommand(robo_id, move1, move2)
            self.zero_flag = 0
        elif self.zero_flag == 0:
            robo_id = self.db.getRoboId(ROBO_NAME, str(msg.topic))
            self.db.writeKommand(robo_id, move1, move2) 
            self.zero_flag = 1          
